...
namespace Open.Caching.Web
{
    public struct CachingPolicy
    {
        ...
        public void Insert<T>(string key, T value, CacheDependency dependencies = null, CacheItemRemovedCallback callback = null)
            => HttpRuntime.Cache?.Insert(key, value, dependencies, ExpiresAbsolute, ExpiresSliding, Priority, callback);
        public void Insert<T>(string key, T value, CacheItemRemovedCallback callback)
            => Insert(key, value, null, callback);

        public object Add<T>(string key, T value, CacheDependency dependencies = null, CacheItemRemovedCallback callback = null)
            => HttpRuntime.Cache?.Add(key, value, dependencies, ExpiresAbsolute, ExpiresSliding, Priority, callback);
        public object Add<T>(string key, T value, CacheItemRemovedCallback callback)
        ...
        public Task<T> GetOrAddAsync<T>(string key, Func<T> factory, bool runSynchronously = false)
            => GetOrAddAsync(key, factory, null, runSynchronously);


        public Task<T> GetOrAddAsync<T>(string key, Func<T> factory, Func<CacheDependency> dependencies, bool runSynchronously = false)
        {
            ...
            current = ReturnAsTask<T>(Add(key, task, dependencies?.Invoke()));
        ...
        public Task<T> GetOrAddAsync<T>(string key, Func<Task<T>> factory, Func<CacheDependency> dependencies = null)
        {
            // Check if task/value exists first.
            var current = ReturnAsTask<T>(HttpRuntime.Cache?[key]);
            if (current != null) return current;
            ...
            // Attempt to add the task.  If current is not null, then return it (not ours).
            current = ReturnAsTask<T>(Add(key, task.Unwrap(), dependencies?.Invoke()));
        ...
        public T GetOrAdd<T>(string key, Func<T> factory, Func<CacheDependency> dependencies = null)
            => GetOrAddAsync(key, factory, dependencies, true).Result;

        public T GetOrAdd<T>(string key, Func<T> factory, Func<string> dependencies)
            => GetOrAdd(key, factory, dependencies == null ? (Func<CacheDependency>)null : () => new CacheDependency(dependencies()));

        public T GetOrAdd<T>(string key, Func<T> factory, Lazy<string> dependencies)
            => GetOrAdd(key, factory, dependencies == null ? (Func<string>)null : () => dependencies.Value);
    }
}
